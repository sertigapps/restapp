...
export class SubCategory {
  ...
  save(emailaddress,token){
    ...
    return this.http.post('https://nopmb791la.execute-api.us-east-1.amazonaws.com/devapp/subcategory', JSON.stringify(data), options)
    .map(res => res.json());
    }
    else{
      var attr_to_change = {};
      ...
      if(counter){
          ...
          headers.append('sertig_token',token.toString());
          headers.append('sertig_email',emailaddress.toString());
        return this.http.put('https://nopmb791la.execute-api.us-east-1.amazonaws.com/devapp/subcategory/id/'+this.id, JSON.stringify(attr_to_change), options)
        .map(res => res.json());
      }
      else{
  ...
  delete(emailaddress,token){
        ...
        headers.append('sertig_token',token.toString());
        headers.append('sertig_email',emailaddress.toString());
            return this.http.delete('https://nopmb791la.execute-api.us-east-1.amazonaws.com/devapp/subcategory/id/'+this.id,  options)
            .map(res => res.json());
  }
}