...
export class ModalIngredientPage {
  ...
  submit(){
    ...
    content: this.translate.transform('saving')+' '+this.translate.transform('ingredient'),
  });
    ...
    this.ingredient.save(this.userprovider.emailaddress,this.userprovider.token).subscribe(res => {
      ...
      if (!res.errorMessage) {
        if(this.id===0){
          ...
          this.menuprovider.orderingredients();
        }
        else{
          if(res.Attributes){
...
public uploadImage() {
  ...
  fileTransfer.upload(targetPath, url, options).then(data => {
    ...
    if(this.ingredient.full_record.image_url && this.ingredient.full_record.image_url!=''){
      ...
      let data = {item:this.ingredient.full_record.image_url.split('/').pop(),appid:'labarraapp'};
