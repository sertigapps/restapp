...
@IonicPage()
@Component({
  selector: 'page-modal-subcategorie',
  templateUrl: 'modal-subcategorie.html',
})
export class ModalSubcategoriePage {
  ...
  this.loading.present();
    this.scategory.save(this.userprovider.emailaddress,this.userprovider.token).subscribe(res => {
      ...
      if (!res.errorMessage) {
        if(this.id===0){
          this.menuprovider.sub_categories.push( new SubCategory(res.id,res.category_id,res.name,res,this.http));
        }
        else{
          if(res.Attributes){
            this.menuprovider.updatesubcategory(this.scategory.id,res.Attributes);
          }
        }
        this.viewCtrl.dismiss();
      } else {
...
public uploadImage() {
 ...
 let options_mage = {
  ...
  fileTransfer.upload(targetPath, url, options).then(data => {
    ...
    if(this.scategory.full_record.image_url && this.scategory.full_record.image_url!=''){
      ...
      let data = {item:this.scategory.full_record.image_url.split('/').pop(),appid:'labarraapp'};
